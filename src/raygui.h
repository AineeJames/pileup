 ...
 *                         REDESIGNED: Global alpha consideration moved to
 *GuiDrawRectangle() and GuiDrawText() REDESIGNED: GuiScrollPanel(), get
 *parameters by reference and return result value REDESIGNED: GuiToggleGroup(),
 *get parameters by reference and return result value REDESIGNED: GuiComboBox(),
 *get parameters by reference and return result value REDESIGNED: GuiCheckBox(),
 *get parameters by reference and return result value REDESIGNED: GuiSlider(),
 *get parameters by reference and return result value REDESIGNED:
 *GuiSliderBar(), get parameters by reference and return result value
 *                         REDESIGNED: GuiProgressBar(), get parameters by
 *reference and return result value REDESIGNED: GuiListView(), get parameters by
 *reference and return result value REDESIGNED: GuiColorPicker(), get parameters
 *by reference and return result value REDESIGNED: GuiColorPanel(), get
 *parameters by reference and return result value REDESIGNED:
 *GuiColorBarAlpha(), get parameters by reference and return result value
 *                         REDESIGNED: GuiColorBarHue(), get parameters by
 *reference and return result value REDESIGNED: GuiGrid(), get parameters by
...
int GuiDropdownBox(Rectangle bounds, const char *text, int *active,
  ...
  GuiDrawText(items[itemSelected], GetTextBounds(DEFAULT, bounds),
...
#define RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY                                       \
  ...
  if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging &&
      (GuiGetStyle(TEXTBOX, TEXT_READONLY) == 0) && (wrapMode == 0)) {
    Vector2 mousePoint = GetMousePosition();
...
#define ICON_TEXT_PADDING 4
  ...
  int wrapMode = GuiGetStyle(
      TEXTBOX, TEXT_WRAP_MODE); // Wrap-mode only available in read-only mode,
                                // no for text editing
  int alignmentVertical = GuiGetStyle(TEXTBOX, TEXT_ALIGNMENT_VERTICAL);
  ...
  for (int i = 0; i < lineCount; i++) {
    ...
    case TEXT_ALIGN_LEFT:
      boundsPos.x = bounds.x;
      break;
    case TEXT_ALIGN_CENTER:
      boundsPos.x = bounds.x + bounds.width / 2 - textSizeX / 2;
      break;
    case TEXT_ALIGN_RIGHT:
      boundsPos.x = bounds.x + bounds.width - textSizeX;
      break;
    default:
      break;
    }
    ...
    case 0:
      boundsPos.y = bounds.y + posOffsetY + bounds.height / 2 -
                    totalHeight / 2 + TEXT_VALIGN_PIXEL_OFFSET(bounds.height);
      break; // CENTERED
    case 1:
      boundsPos.y = bounds.y + posOffsetY;
      break; // UP
    case 2:
      boundsPos.y = bounds.y + posOffsetY + bounds.height - totalHeight +
    ...
    if (iconId >= 0) {
      ...
      GuiDrawIcon(iconId, (int)boundsPos.x,
            ...
            (codepoint != '\t')) // Do not draw codepoints with no glyph
        {
          if (wrapMode == 0) // 0-NO_WRAP
          {
            ...
            if (textOffsetX <= (bounds.width - glyphWidth)) {
              DrawTextCodepoint(
                  guiFont, codepoint,
                  RAYGUI_CLITERAL(Vector2){boundsPos.x + textOffsetX,
                                           boundsPos.y + textOffsetY},
          ...
          } else if (wrapMode >= 1) // 2-WORD_WRAP
          {
            ...
            if ((boundsPos.y + textOffsetY) <=
                (bounds.y + bounds.height - GuiGetStyle(DEFAULT, TEXT_SIZE))) {
              DrawTextCodepoint(
                  guiFont, codepoint,
                  RAYGUI_CLITERAL(Vector2){boundsPos.x + textOffsetX,
                                           boundsPos.y + textOffsetY},
